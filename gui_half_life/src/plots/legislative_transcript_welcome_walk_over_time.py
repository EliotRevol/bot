...
from gui.src.utils import read_transcript, check_with_accent, multi_check_with_accent
...
def generate_plots(experiment_files_path, standardize=True):
    files = [a for a in glob.glob(os.path.join(experiment_files_path, "*")) if
             Experiments.NATIONAL_NEWS_WALK in a and ResultTypes.TRANSCRIPT in a]
    ...
    if standardize:
        path_for_jsons = "gui_data/plot_data/legislative_transcript_welcome_walk_over_time/*.json"
    else:
        path_for_jsons = "gui_data/plot_data/legislative_transcript_welcome_walk_over_time_abs/*.json"
    ...
    if len(dates) > 0:
            ...
            for exp, date in tqdm(valid_exps_dates):
                if date in result_date_dict:
                    for candidate,snonyms in LegislativeParties.Parties.items():
                        ...
                        durations = sum(
                            [duration for text, duration in video_titles if multi_check_with_accent([candidate] + snonyms, text.lower())])
        ...
        for date in result_date_dict.keys():
            ...
            if standardize:
                output = get_path(
                    f"gui_data/plot_data/legislative_transcript_welcome_walk_over_time/{date}.json")
            else:
                output = get_path(
                    f"gui_data/plot_data/legislative_transcript_welcome_walk_over_time_abs/{date}.json")
