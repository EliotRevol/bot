...
from gui.src.plots.consts import Candidates, Experiments, ResultTypes
...
def title_check(candidates_list, titles, urls):
    ...
    for candidate in candidate_dictionary.keys():
        ...
        videos = [e for a, d in zip(titles, urls) for b, e in zip(a, d) if check_with_accent(candidate_lower, b)]
...
def generate_plots(experiment_files_path, last_pull_date):
    ...
    result, political_value, candidate_url_dictionary = title_check(Candidates.official, titles, urls)
    most_common_videos = {}
    ...
    #                       for a, b in candidate_url_dictionary.items()}
    output = get_path(
        f"gui_data/plot_data/welcome_title_ratio/{last_pull_date}__{int(to_date)}__{int(datetime.datetime.timestamp(datetime.datetime.now()))}.json")
...
if __name__ == '__main__':
    ...
    generate_plots("/media/ali/30aa46bd-4e3b-4772-b005-8cfe085f9148/home/ali/election_data/*/*", "03_24_2022")
