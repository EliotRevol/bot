...
from core.src.utils import dump_json
from gui.src.plots.consts import ResultTypes, Experiments, Candidates
from gui.src.utils import read_transcript, check_with_accent
...
def parse_filename(filename):
    return datetime.datetime.fromtimestamp(int(filename.split("_")[-2]))
...
def generate_plots(experiment_files_path, standardize=True):
    files = [a for a in glob.glob(os.path.join(experiment_files_path, "*")) if
             Experiments.WELCOME_WALK in a and ResultTypes.TRANSCRIPT in a]
    ...
    if standardize:
        path_for_jsons = "gui_data/plot_data/transcript_welcome_walk_over_time/*.json"
    else:
        path_for_jsons = "gui_data/plot_data/transcript_welcome_walk_over_time_abs/*.json"
    ...
    if len(dates) > 0:
        ...
        for date in result_date_dict.keys():
            ...
            if standardize:
                output = get_path(
                    f"gui_data/plot_data/transcript_welcome_walk_over_time/{date}.json")
            else:
                output = get_path(
                    f"gui_data/plot_data/transcript_welcome_walk_over_time_abs/{date}.json")
...
if __name__ == '__main__':
    ...
    generate_plots("/media/ali/30aa46bd-4e3b-4772-b005-8cfe085f9148/home/ali/election_data/*/*", standardize=False)
