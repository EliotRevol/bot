...
from gui.src.utils import check_with_accent
...
def generate_plots(experiment_files_path, standardize=True):
    files = [a for a in glob.glob(os.path.join(experiment_files_path, "*")) if
             Experiments.WELCOME_FETCH in a and ResultTypes.JSON in a]
    ...
    if standardize:
        path_for_jsons = "gui_data/plot_data/title_welcome_fetch_over_time/*.json"
    else:
        path_for_jsons = "gui_data/plot_data/title_welcome_fetch_over_time_abs/*.json"
    ...
    if len(dates) > 0:
                    ...
                    for candidate in Candidates.official:
                        ...
                        title_count = sum(
                            [1 for title in exp if
                             check_with_accent(candidate_lower, title.lower())])
        ...
        for date in result_date_dict.keys():
            ...
            if standardize:
                output = get_path(
                    f"gui_data/plot_data/title_welcome_fetch_over_time/{date}.json")
            else:
                output = get_path(
                    f"gui_data/plot_data/title_welcome_fetch_over_time_abs/{date}.json")
