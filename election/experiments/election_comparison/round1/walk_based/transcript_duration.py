...
from utils.const import ResultTypes, Experiments, Candidates, Results, Polls, Folders
    ...
    if generate:
        for exp in [Experiments.WELCOME_WALK,
            ...
            for month, folders in tqdm.tqdm(Folders.Round1.folders_dict.items()):
                ...
                experiments_json = read_multi_folder(folders, exp, ResultTypes.TRANSCRIPT, path
                                                     )
                ...
                for candidate in tqdm.tqdm(candidate_dictionary.keys()):
                    candidate_lower = candidate.lower()
                    candidate_dictionary[candidate] += sum(
                        [a[1] for a in video_titles.items() if
                         check_with_accent(candidate_lower, a[0].lower())])
                ...
                dump_pickle("transcript_duration_" + exp.lower() + "_" + month + ".pkl", candidate_dictionary)
    plot = True
    if plot:
        for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
            ...
            for month, folders in Folders.Round1.folders_dict.items():
                ...
                file_name = re.sub(r"[\n\t\s]*", "", plot_title_combined)
                candidate_dictionary = read_pickle("transcript_duration_" + exp.lower() + "_" + month + ".pkl")
                ...
                plt.tight_layout()
                plt.savefig(
                    "../../../../plots/election_comparison/round1/walk_based/transcript_duration_" + file_name + "_" + month + ".png")
