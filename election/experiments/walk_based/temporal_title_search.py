...
if __name__ == '__main__':
    ...
    if generate:
        ...
        for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
            ...
            # Official Candidates
            candidate_dictionary = {c_: {b: 0 for b in set([a[1] for a in video_titles if a[1] and a[1] != ""])} for c_
                                    in
                                    Candidates.official}
    ...
    if plot:
        for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
            ...
            plt.tight_layout()
            plt.savefig(
                "../../plots/walk_based/temporal_title_search/temporal_title_search_" + file_name + "_" + ".png")
            df = pd.DataFrame.from_dict(candidate_dictionary)
            df.sort_index(inplace=True)
            df.rolling(window=7).mean().plot(title=plot_title_combined, figsize=(15, 7))
            plt.tight_layout()
            plt.savefig(
                "../../plots/walk_based/temporal_title_search/temporal_title_search_ma7_" + file_name + ".png")
            plt.show()
            #
            #
            # # plt.show()
