...
if __name__ == '__main__':
    ...
    #     candidate_dictionary = read_pickle("temporal_transcript_duration_" + exp.lower() + "_" + ".pkl")
    ...
    #
    #     plt.savefig(
    #         "../../plots/walk_based/temporal_transcript_duration/temporal_transcript_duration_vs_polls_ma7_" + file_name + ".png")
    #     # corr = df_merged.corr()
    #     # fig = plt.figure()
    ...
    for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
        ...
        df_merged_unrolled = df_merged.copy(deep=True)
        ...
        df_polls_plot.to_csv("../../plots/walk_based/temporal_transcript_duration/df_polls.csv")
        ax.set_prop_cycle(None)
        df_yt_plot = df_yt_plot[sorted(df_yt_plot)]
        df_yt_plot.plot(ax=ax, linestyle='--', lw=2)
        df_yt_plot.to_csv("../../plots/walk_based/temporal_transcript_duration/df_yt.csv")
        ax.set_prop_cycle(None)
        colors = []
        normalized_elections = {a: b / sum(Results.round1.values()) for a, b in Results.round1.items()}
        pd.DataFrame(normalized_elections, index=["2022-04-10"]).to_csv(
            "../../plots/walk_based/temporal_transcript_duration/election_results_normalized.csv")
        ...
        plt.savefig(
            "../../plots/walk_based/temporal_transcript_duration/temporal_transcript_duration_vs_polls_ma7_" + file_name + ".pdf")
        # corr = df_merged.corr()
        # fig = plt.figure()
        # ax = fig.add_subplot(111)
        # cax = ax.matshow(corr, cmap='coolwarm', vmin=-1, vmax=1)
        ...
        for c in df_yt_plot_v2.columns:
            ...
            rmse = mean_absolute_error(polls_plot * 100, yt_plot * 100)
            avg += rmse
            print(f"{rmse :.2f}\t{mean_absolute_error(polls_plot, yt_plot):.2f}")
        ...
        for a in avg_score:
            ...
            print(f"{a:.2f}")
        print(f"{avg_per_day_mention_score / len(avg_score):.2f}")

        df_merged_unrolled

        df_yt_plot_v3 = df_merged_unrolled[
            [a for a in df_merged_unrolled.columns if
             'YouTube' in a and a.split("_")[0] in Candidates.official]]
        df_yt_plot_v3 = df_yt_plot_v3.rename(
            columns={a: a.split("_")[0] for a in df_yt_plot_v3.columns})
        df_yt_plot_v3 = df_yt_plot_v3[sorted(df_yt_plot_v3)]

        df_polls_plot_v3 = df_merged_unrolled[
            [a for a in df_merged_unrolled.columns if
             'Polls' in a and a.split("_")[0] in Candidates.official]]
        df_polls_plot_v3 = df_polls_plot_v3.rename(
            columns={a: a.split("_")[0] for a in df_polls_plot_v3.columns})
        df_polls_plot_v3 = df_polls_plot_v3[sorted(df_polls_plot_v3)]
        _, election_results = zip(*sorted(zip(normalized_elections.keys(), normalized_elections.values())))
        print(
            f"Last MAE result vs poll: {mean_absolute_error([100 * a for a in election_results], df_polls_plot_v3.dropna().iloc[-1] * 100)}")
        print(
            f"Last MAE results vs yt: {mean_absolute_error([100 * a for a in election_results], df_yt_plot_v3.iloc[-1] * 100)}")
        # for c in df_yt_plot_v2.columns:
        #     print(c)
        #     result = adfuller(df_polls_plot_v2[c].diff().dropna())
        ...
        #
        #     result = adfuller(df_yt_plot_v2[c].diff().dropna())
        #     print('ADF Statistics: %f' % result[0])
        #     print('p-value: %f' % result[1])
        #     print('Critical values:')
        #     for key, value in result[4].items():
        #         print('\t%s: %.3f' % (key, value))
        #
        #     print(grangercausalitytests(pd.DataFrame(pd.concat([df_polls_plot_v2[c].diff().dropna(), df_yt_plot_v2[c].diff().dropna()], axis=1)),
        #                                 maxlag=[3]))
