...
if __name__ == '__main__':
    ...
    if generate:
        ...
        for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
            ...
            dump_pickle("temporal_transcript_duration_" + exp.lower() + "_" + ".pkl", candidate_dictionary)
    plot = True
    if plot:
        for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
            print(exp)
            ...
            plt.tight_layout()
            plt.savefig(
                "../../plots/walk_based/temporal_transcript_duration/temporal_transcript_duration_" + file_name + "_" + ".png")

            df = pd.DataFrame.from_dict(candidate_dictionary)
            df.sort_index(inplace=True)
            df.rolling(window=7).mean().plot(title=plot_title_combined, figsize=(15, 7))
            plt.tight_layout()
            plt.savefig("../../plots/walk_based/temporal_transcript_duration/temporal_transcript_duration_ma7_" + file_name + ".png")
            plt.show()
    print("Finished")
