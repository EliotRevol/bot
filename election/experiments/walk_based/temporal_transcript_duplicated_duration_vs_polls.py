...
if __name__ == '__main__':
    ...
    for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
        ...
        avg_mention_of_candidate_dict = {
            a: sum([b['duration'] for c, b in v.items()]) / sum([b['count'] for c, b in v.items()]) for a, v in
            candidate_dictionary.items()}
        candidate_dictionary = {a: {b: c['duration'] / c['count'] if c['count'] != 0 else 0 for b, c in v.items()} for
                                a, v in
                                candidate_dictionary.items()}
        ...
        plt.savefig(
            "../../plots/walk_based/temporal_transcript_duplicated_duration/temporal_transcript_duplicated_duration_vs_polls_ma7_" + file_name + ".pdf")
        # corr = df_merged.corr()
        # fig = plt.figure()
        # ax = fig.add_subplot(111)
        # cax = ax.matshow(corr, cmap='coolwarm', vmin=-1, vmax=1)
        ...
        print("Avg speaking")
        avg_mention=0
        for a in sorted(avg_mention_of_candidate_dict):
            a_ = avg_mention_of_candidate_dict[a]
            print(f"{a_ :.2f}")
            avg_mention+=a_
        print(f"{avg_mention/len(avg_mention_of_candidate_dict):.2f}")

        _,election_results=zip(*sorted(zip(normalized_elections.keys(), normalized_elections.values())))
        print(f"Last MAE result vs poll: {mean_absolute_error(election_results,df_polls_plot_v2.iloc[-1])}")
        print(f"Last MAE results vs yt: {mean_absolute_error(election_results,df_yt_plot_v2.iloc[-1])}")