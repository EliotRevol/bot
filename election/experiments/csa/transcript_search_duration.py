...
if __name__ == '__main__':
    ...
    for exp in [Experiments.WELCOME_WALK,
        ...
        experiments_json = read_multi_folder(experiment_folder, exp, ResultTypes.TRANSCRIPT,"/media/ali/30aa46bd-4e3b-4772-b005-8cfe085f9148/home/ali/election_data/")
        # url_tag_dict = {d["url"]: "".join([a['text'] for a in d['transcript']]) for d in experiments_json}

        video_titles = [(exp['text'], exp['duration']) for e in experiments_json for exp in e['transcript']]
        # Polls
        ...
        for candidate1 in tqdm(candidate_dictionary.keys()):
            ...
            durations = sum(
                [duration for text, duration in video_titles if check_with_accent(candidate_lower, text.lower())])
        ...
        plt.tight_layout()
        file_name = re.sub(r"[\n\t\s]*", "", plot_title_combined)
        plt.savefig("../../plots/csa/transcript_search_duration_" + file_name +"_"+ str(CAND)+".png")
