...
if __name__ == '__main__':
    ...
    csa_df = pd.read_csv("54TP-TA_Presidentielle_2022__28 mars au 3 avril 2022.csv", sep=";")

    csa_df = csa_df[csa_df['Type de temps'].isin(["Candidat", "Soutiens", "Antenne"])]
    duration_cols = [a for a in csa_df.columns if a.startswith("Durée")]
    for d in duration_cols:
        csa_df[d] = csa_df[d].apply(lambda x: get_time(x))
    csa_df['Hours'] = csa_df[duration_cols].sum(axis=1)
    # experiment_folder = ["14.01.2022", "07_01_2022"]
    ...
    dump_pickle(
        f"title_search_csa_dict_28-03__08-04_{int(datetime.datetime.timestamp(datetime.datetime.now()))}.pkl",
        csa_dict)
