...
from utils.io import read_multi_folder, read_transcript, read_json_gz, read_json, dump_pickle, read_pickle
...
if __name__ == '__main__':
    ...
    if generation:
        ...
        for exp in [#Experiments.WELCOME_WALK,
            ...
            # url - text
            video_titles = [(e['url'], [(exp['text'], exp['duration']) for exp in e['transcript']]) for e in
                            experiments_json
            ...
            dump_pickle("csa_per_tv_1_01__13_02" + str(exp) + ".pkl", normalized_tv_dict)

    plot = True
    if plot:
        for exp in [Experiments.WELCOME_WALK,
                    Experiments.NATIONAL_NEWS_WALK]:
            normalized_tv_dict = read_pickle("csa_per_tv_1_01__13_02" + str(exp) + ".pkl")
            for channels in normalized_tv_dict.keys():
                ...
                plt.xticks(rotation=90)
                plt.tight_layout()
                plt.savefig("../../plots/csa/channels/" + str(exp.lower()) + "/" + channels + ".png")
                plt.cla()
