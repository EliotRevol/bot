...
from utils.io import dump_pickle, read_pickle, read_transcript, dump_json
...
if __name__ == '__main__':
    ...
    if generate:
        for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
            ...
            [experiments_json.extend(
                [(a1, f1.split("/")[-2]) for a1 in read_transcript(f1) if a1['status'] == 'Status.OK']) for f1 in
                tqdm.tqdm(files)
                if
            ...
            video_titles = {"".join(exp['text']).lower(): (exp['duration'], e[1]) for e in experiments_json for exp in
                            e[0]['transcript']}  # change this to list

            # Official Candidates
            candidate_dictionary = {
                c_: {b: 0 for b in set([a[1] for a in video_titles.values() if a[1] and a[1] != ""])} for c_
                in
                partySynonymType}
            for candidate, snonyms in tqdm.tqdm(partySynonymType.items()):
                ...
                for text, date in video_titles.items():
                    if multi_check_with_accent([candidate] + snonyms, text.lower()):
                        candidate_dictionary[candidate][date[1]] += date[0]

            dump_pickle("temporal_transcript_duration_" + exp.lower() + "_" + partySynonymTypeStr + ".pkl",
                        candidate_dictionary)
    plot = True
    if plot:
        for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
            ...
            candidate_dictionary = read_pickle(
                "temporal_transcript_duration_" + exp.lower() + "_" + partySynonymTypeStr + ".pkl")
            ...
            plt.tight_layout()
            plt.savefig(
                "../../plots/legislative/walk_based/temporal_transcript_duration/temporal_transcript_duration_" + file_name + "_" + partySynonymTypeStr + ".png")
            dump_json(
                "../../plots/legislative/walk_based/temporal_transcript_duration/temporal_transcript_duration_" + file_name + "_" + partySynonymTypeStr + ".json",
                candidate_dictionary)
            df = pd.DataFrame.from_dict(candidate_dictionary)
            ...
            plt.tight_layout()
            plt.savefig(
                "../../plots/legislative/walk_based/temporal_transcript_duration/temporal_transcript_duration_ma7_" + file_name + "_" + partySynonymTypeStr + ".png")
            # plt.show()
    ...
    if polls_comparison:
        for exp in [Experiments.WELCOME_WALK, Experiments.NATIONAL_NEWS_WALK]:
            ...
            plt.savefig(
                "../../plots/legislative/walk_based/temporal_transcript_duration/temporal_transcript_duration_vs_polls_" + file_name + "_" + partySynonymTypeStr + ".png")
            # ----- corr mae etc ---
